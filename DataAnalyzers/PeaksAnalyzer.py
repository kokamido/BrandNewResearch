...
def calc_peacks(points: NDArray[np.float64], min_amplitude: float = 0.0001) -> Dict[str, Union[float, str, None]]:
    ...
    for i in range(2, len(points) - 3):
        if (points[i + 1] - points[i]) / derivative_sign < 0:
            derivative_sign *= -1
            half_picks += 1
    return {'peaks': half_picks / 2 + (0.5 if half_picks >= 0 else 0),
