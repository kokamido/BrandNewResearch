...
from DataAnalyzers.PeaksAnalyzer import add_peaks_stats_Higgins1D, add_amp_stats_Higgins1D, calc_peacks, calc_Fourier_coeff_for_transient
...
def draw_transient(e: Experiment, left_border: float = None, right_border: float = None,
    ...
    ax = draw_transient_internal(
        e.timelines['u'].T[::-1, left_border:right_border], dt, dx, '$t$', '$U$', cmap, xticklabels, ytickslabels, ax)
    if not draw_u_only:
        ax = draw_transient_internal(
            e.timelines['v'].T[::-1, left_border:right_border], dt,dx, '$t$', '$V$', cmap, xticklabels, ytickslabels, ax)
    return ax


def draw_transient_internal(data: ndarray, dt: float, dx: float, x_name: str, y_name: str, cmap: str,
...
def draw_arr_WxH(res: List[Experiment], left_border: float = None, right_border: float = None, w: int = 3, h: int = 3) -> Tuple[figure, List[axes]]:
    ...
    title = f"$p={ex.model_config['p']}, q={ex.model_config['q']}, D_u={ex.model_config['Du']}, init={calc_peacks(ex.init_values['u'])}$"
    fig.suptitle(title)
