...
BASE_METHOD_CONF = ExplicitParameters2D(None, None, d_h, d_t, 5000, 2, timeline_save_points, 0.0)
RES_FOLDER = '/media/alexander/BigDisk/data/selk_strogg_2D_2023_02_26_long'
...
def save(folder, config_model, config_method, res_u, res_v):
    ...
    for i in range(1, len(res_u)):
        res_u[i] = res_u[i].round(2)
        res_v[i] = res_v[i].round(2)
...
for a in np.linspace(0.0025,0.1225, 13):
    for i in range(10):
        to_calc.append((a, 1.0, BASE_MODEL_CONF, BASE_METHOD_CONF, f'{RES_FOLDER}/{a}_{1.0}_{i}'))
